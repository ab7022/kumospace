...
import { MessageSquare, Plus, Search, Send, Workflow } from "lucide-react";
  ...
  useEffect(() => {
    if (chatMember && email) {
      const filtered = allMessages.filter(
        (msg) =>
          (msg.senderEmail === email && msg.recipientEmail === chatMember.email) ||
          (msg.senderEmail === chatMember.email && msg.recipientEmail === email)
      );
      setFilteredMessages(filtered);
      scrollToBottom();
    }
  }, [chatMember, allMessages, email]);
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen w-screen bg-gray-50">
      ...
      <div className="w-full lg:w-80 bg-white border-r border-gray-200">
        ...
        <div className="p-4">
          <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Team</h2>
          <div className="mt-3 space-y-2">
            <div className="p-3 bg-gradient-to-r from-indigo-50 to-transparent border-l-4 border-indigo-500 rounded-lg">
              <div className="flex items-center">
                ...
                <div className="ml-3">
                  <p className="text-sm font-medium text-gray-900">Team Space</p>
                  <p className="text-xs text-gray-500 truncate">General Discussion</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="p-4">
          <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">Personal</h2>
            ...
            {teamMembers?.map((member, index) => (
              <div
                ...
                className={`p-3 rounded-lg cursor-pointer transition-all hover:bg-gray-50 ${
                  chatMember?.id === member.id ? "bg-gray-50" : ""
                }`}
              >
                <div className="flex items-center">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <div className="relative">
                  <Image
                    src={chatMember.profileUrl || `https://ui-avatars.com/api/?name=${chatMember.name}&background=6366f1&color=fff`}
                    alt={chatMember.name}
                    width={48}
                    height={48}
                    className="rounded-full"
                  ...
                  <span 
                    className={`absolute bottom-0 right-0 block h-3 w-3 rounded-full ring-2 ring-white ${
                      chatMember.status === "AVAILABLE" ? "bg-green-400" :
                      chatMember.status === "BUSY" || chatMember.status === "DND" ? "bg-yellow-400" :
                      "bg-gray-400"
                    }`}
                  />
                </div>
                <div className="ml-3">
                  <h2 className="text-lg font-medium text-gray-900">{chatMember.name}</h2>
        ...
        <div className="flex-1 p-4 overflow-y-auto bg-gradient-to-b from-gray-50 to-white">
          <div className="space-y-4">
            {filteredMessages.map((msg, index) => (
              ...
              >
                <div className={`flex items-end gap-2 max-w-[70%] ${msg.senderEmail === email ? 'flex-row-reverse' : ''}`}>
                  <Image
                    src={msg.image || `https://ui-avatars.com/api/?name=${msg.name}`}
                    alt={msg.senderEmail}
                    width={32}
                    height={32}
                    className="rounded-full"
